...
(
	@LinkedServiceType VARCHAR(140)
	,@AuthenticationType VARCHAR(200)
	,@LinkedServiceDesc VARCHAR(140)
)
AS
BEGIN
	...
	SELECT	@ListLinkedServiceId = LinkedServiceId  
	FROM	[dbo].[T_List_LinkedServices] 
	WHERE	LinkedServiceName = @LinkedServiceType 
			AND AuthenticationType = @AuthenticationType

	IF (@AlreadyExists = 0)
	BEGIN
		INSERT INTO [dbo].[T_Pipeline_LinkedServices] 
		(
				LinkedServiceId
				,LinkedServiceName
		) 
